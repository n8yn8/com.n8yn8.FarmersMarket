...
public class DatabaseHelper extends SQLiteOpenHelper {
 ...
 // Markets table create statement
     ...
     * getting all Markets under single vendor
     * */
    public List<Vendor> getAllVendorsAtMarket(String market_name) {
        List<Vendor> vendors = new ArrayList<Vendor>();
     
        String selectQuery = "SELECT  * FROM " + TABLE_VENDORS + " td, "
                + TABLE_MARKETS + " tg, " + TABLE_VENDORS_AT_MARKETS + " tt WHERE tg."
                + KEY_MARKET_NAME + " = '" + market_name + "'" + " AND tg." + KEY_ID
     ...
     * Deleting a vendor
     */
    public void deleteVendor(Vendor vendor, boolean should_delete_all_vendor_markets) {
        SQLiteDatabase db = this.getWritableDatabase();
     
        // before deleting vendor
        // check if markets under this vendor should also be deleted
        if (should_delete_all_vendor_markets) {
            // get all markets under this vendor
            List<Market> allVendorToDos = getAllMarketsByVendor(vendor.getName());
     
            // delete all markets
            for (Market market : allVendorToDos) {
                // delete market
                deleteMarket(market.getId());
            }
        }
    ...
    public Item getItem(long item_id) {
        ...
        item.setVendor(c.getString(c.getColumnIndex(KEY_VENDOR)));
